...
module.exports = {
    ...
    return new Promise(
      function (resolve, reject) {
        this.refreshAuthFromStore(
          function () {
            if (
              (!_.isUndefined(this.authObject) &&
                !_.isUndefined(this.authObject.accessToken)) ||
              this._hasClientSideRedirect()
            ) {
              resolve();
            } else {
              reject();
  ...
  getAccessToken: function (authCode, redirectURL, callback) {
    if (this.authType !== "server") {
      callback(
        new Error(
          "In authentication types other than server access token is delivered through a redirect"
        )
      );
  ...
  getAuthorizationURL: function (redirectURL) {
    ...
    switch (this.authType) {
      ...
      case "password":
        throw new Error(
          "Authorization URLs are not supported for password authentication"
        );
    }
  ...
  authenticateWithCredentialsForOffering: function (
    username,
    password,
    offeringId,
    callback
  ...
  authenticateWithActivationCodeForOffering: function (
    activationCode,
    offeringId,
    callback
  ) {
  ...
  _authRequest: function (url, requestData, callback) {
    request
      ...
      .send(requestData)
      .end(
        this._onAuthResponse.bind(this, callback, requestData.grant_type, url)
      );
  },
  ...
  _onAuthResponse: function (callback, grantType, url, err, res) {
    ...
    if (res.ok && !err) {
      ...
      callback(
        new PodioErrors.PodioAuthorizationError(
          "Authentication for " +
            grantType +
            " failed. Reason: " +
